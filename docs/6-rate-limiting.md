...
The rate limiter is designed to fail open or degrade gracefully.

### Redis Outage

If Redis becomes unreachable:

1.  The middleware detects the connection failure via `isRedisHealthy()`.
2.  It logs a warning: `[RateLimit] Redis unhealthy, falling back to in-memory store`.
3.  It automatically switches to an **In-Memory Store** for the duration of the outage.
4.  **Impact**: Rate limits will be enforced per-instance (not distributed), but the API remains protected and operational.
