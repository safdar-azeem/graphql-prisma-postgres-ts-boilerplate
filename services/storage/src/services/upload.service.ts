...
export const createSignedUploadUrl = async (
  ...
  // SEC-4: Strip internal storage details in proxy mode to prevent leaking bucket paths
  if (FILE_PROXY_MODE) {
    return {
      signedUrl: signedUrlResult.signedUrl,
      fileId: file.id,
      publicUrl: '', // Hidden in proxy mode
      storageKey: '', // Hidden in proxy mode
      expiresAt: signedUrlResult.expiresAt,
    }
  }
...
export const confirmUpload = async (fileId: string, ownerId: string): Promise<File> => {
  ...
  // FIX: Explicitly set the file permissions after confirming existence
  // This ensures that even if the client didn't send ACL headers, the file becomes public if needed
  await provider.setFileVisibility(file.storageKey, file.isPublic)
